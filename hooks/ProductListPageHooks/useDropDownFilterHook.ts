...
// working fine please check previous code and compare 
...
const useDropDownFilterHook = () => {
  ...
  // Handle dropdown filter selection
  const handleFilterSelectDropDown = (options: any[], meta: any) => {
    setSelectedFilters((prevFilters) => {
      let updatedFilters = [...prevFilters];

      // Process each option in the array
      options.forEach(option => {
        const section = option?.section || meta?.name || '';
        ...
        if (existingSectionIndex !== -1) {
          ...
          if (isSelected) {
            ...
            // Select: Add the value
            updatedFilters[existingSectionIndex].value.push(selectedValue);
          }
        } else {
          // New section: Add the filter
          updatedFilters.push({ name: section, value: [selectedValue] });
        }
      });
